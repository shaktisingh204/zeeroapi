#!/usr/bin/env python3
"""
melbet page scraper — renders the SPA in real Chrome (via Playwright), extracts
matches + markets *with their real human names*, and POSTs them to the Rust
ingest API. This sidesteps both the anti-bot layer (real browser fingerprint)
and the opaque numeric market codes (the page resolves names itself).

Usage:
    python scrape.py                 # one pass over live + configured sports
    python scrape.py --loop 30       # repeat every 30s
    python scrape.py --headed        # show the browser (debugging)
...
# Map melbet's short outcome codes to friendly (market, outcome) names.
OUTCOME_MAP = {
...
def post_snapshot(source, matches):
    if not matches:
    ...
    try:
        r = httpx.post(
            ...
            json={"source": source, "matches": matches},
            timeout=60,
        )
        r.raise_for_status()
        print(f"  [{source}] sent {len(matches)} matches -> {r.json()}")

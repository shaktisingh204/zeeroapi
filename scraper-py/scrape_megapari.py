#!/usr/bin/env python3
"""megapari page scraper → ZeroApi ingest, tagged provider=megapari.

MegaPari is a 1xbet-family skin running the SAME SPA engine as melbet: identical
DOM (`.dashboard-champ`, `.dashboard-game`, `.ui-market`, `team-score-name`) and
the same anti-bot layer that resets plain `curl` (HTTP 000) but passes a real
browser. So this mirrors `scrape.py`: render the SPA in real Chrome (Playwright,
`channel="chrome"`), scroll the virtualized match list, extract matches + markets
with their real human names from the rendered DOM, then POST to the Rust ingest
API in chunks.

The base domain (`megapari.com`) 302-redirects to a rotating mirror in the
browser; that is transparent here since Playwright follows it. Override via
MEGAPARI_BASE_URL if the canonical entry domain changes.

Usage:
    python scrape_megapari.py                 # one pass over live + configured sports
    python scrape_megapari.py --loop 30       # repeat every 30s
    python scrape_megapari.py --headed        # show the browser (debugging)
    python scrape_megapari.py --dry-run       # extract + print, no POST (no backend needed)

Env:
    MEGAPARI_BASE_URL  default https://megapari.com
...
BASE_URL = os.environ.get("MEGAPARI_BASE_URL", "https://megapari.com").rstrip("/")
PROVIDER = "megapari"
...
TARGETS = [(f"{BASE_URL}/en/live", "live", "megapari-live")] + [
    (f"{BASE_URL}/en/line/{s}", "prematch", f"megapari-{s}") for s in _LINE_SPORTS
]
...
# Map megapari's short outcome codes to friendly (market, outcome) names.
OUTCOME_MAP = {
...
def post_chunks(source, matches):
    """Chunk the snapshot into CHUNK-sized POSTs, tagged provider=megapari."""
    if not matches:
...
def main():
    ...
    ap.add_argument("--dry-run", action="store_true",
                    help="extract and print matches, do not POST to the backend")
    args = ap.parse_args()
